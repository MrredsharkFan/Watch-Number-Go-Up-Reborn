...
function el_automation(dt) { //putting it here because it fits the dt shitposts
    ...
    if (t >= 4) {
        ...
        for (var i = 1; i <= t - (player.exp.gte(10000)?1:4); i++) {
...
function tick() {
    ...
    try {
        ...
        player.exp = player.exp.add(get_experience_gain().times(dt))
        ...
        if (page == 0) {
            ...
            dg("el_qol", `Reset to get 100% of ${get_actual_layer_name(player.exp.gte(1e4)?el:el - 3)}/s!`)
            dgs("el_qol", "visibility", el >= 3||player.exp.gte(1e4) ? "visible" : "hidden")
